...
import PersonnelTimelineView from './PersonnelTimelineView';
...
export default function SupervisionConsultantView({
  ...
  // Active view subtab
  const [activeTab, setActiveTab] = useState<'timeline' | 'personnel' | 'invoices' | 'profile'>('timeline');
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 dark:border-slate-800 pb-2">
        ...
        >
          ...
          Staffing Timeline & Gantt
          <span className={`px-2 py-0.5 rounded-full text-xs font-mono ${
            activeTab === 'timeline' ? 'bg-emerald-700 text-white' : 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-300'
          }`}>
            {personnelSummary.activeCount} Active
          </span>
        </button>
      ...
      {/* TAB 0: INTERACTIVE STAFFING TIMELINE & GANTT VIEW */}
      {activeTab === 'timeline' && (
        <PersonnelTimelineView
          personnel={consultant.personnel || []}
          consultant={consultant}
          projectStartDate={project.startDate || consultant.commencementDate}
          projectCompletionDate={consultant.revisedCompletionDate || consultant.originalCompletionDate}
          onSelectPersonnel={(p) => setViewDetailPersonnel(p)}
          onEditPersonnel={(p) => handleOpenEditPersonnel(p)}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                onClick={() => setActiveTab('timeline')}
                className="px-3 py-1.5 text-xs font-bold rounded-xl bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 flex items-center gap-1.5 hover:bg-emerald-100 dark:hover:bg-emerald-900/50 transition shadow-xs"
              >
