...
export interface IpcMaturationDetails {
  ...
  dueDate: string | null; // Contractor Submission Date + 56 days (FIDIC Sub-Clause 14.7)
  daysElapsed: number; // calendar days counted from Contractor Submission Date
 ...
 * - submissionDate: Contractor Submission Date (date Contractor submitted IPC to Engineer)
 * - certificationDate: Engineer's Submission Date (date Engineer certified & submitted IPC to Employer)
 * - paymentDate: Payment Date / Disbursement Date (date Employer disbursed payment to Contractor)
 */
export function calculateIpcMaturation(
  ...
  if (ipc.submissionDate) {
    ...
    if (!isNaN(subDate.getTime())) {
      ...
      // 56-day FIDIC Sub-Clause 14.7 payment deadline from Contractor Submission Date
  ...
  if (isFullyPaid) {
    statusBadge = {
      label: 'Paid (Settled)',
      subLabel: ipc.paymentDate ? `Paid on ${ipc.paymentDate}` : 'Disbursed / Paid',
      variant: 'paid',
    ...
    statusBadge = {
      label: 'Pending Contractor Submission',
    ...
    statusBadge = {
      ...
      subLabel: `Exceeded 56d payment limit (Day ${daysElapsed})`,
    ...
    statusBadge = {
      ...
      subLabel: `${daysRemaining} days until 56d payment maturity`,
      variant: 'active',
